...
class NeuralGradient(torch.nn.Module):
    ...
    def forward(self, t, y, cov):
        ...
        Args:
            t: the time points, a tensor of shape (batch_size,)
            y: the values of the function at time t, a tensor of shape (batch_size, n_features)
        """
        print(f"Forward call with t = {t}")
        y_cov = torch.cat([y, cov], dim=-1)
        dydt = self.nn(y_cov)
